...
| 2026-07-18 | Dollar-based/fractional orders placed outside a live session (after 4pm ET, or on a weekend) queue as GFD for the next regular session instead of filling immediately; if the routine records a fill (or a no-fill sentinel) using what was knowable AT CYCLE TIME and nothing later reconciles that record against the real eventual fill, the ledger silently drifts from broker truth | Hit twice on real positions: PLTR/HOOD/ASML (opened 2026-07-16 after 4pm ET, actually filled 2026-07-17 09:30 ET at a different price/quantity than the cycle recorded — `3bc566d`) and LLY/META/AMZN (ordered Saturday 2026-07-18, actually filled Monday 2026-07-20 09:30 ET; correctly recorded as no-fill sentinels at cycle time per the first incident's own hardening, but nothing later corrected them once the real fill happened, so the positions were simply missing from the ledger for two days — `d56fa8b`) | Both corrected via the real `PaperLedger` class (never hand-edited JSON) — one (`3bc566d`) via direct field mutation since `apply_buy()` raises `ValueError` on an already-open position, one (`d56fa8b`) via `apply_buy()` after removing the stale no-fill sentinel first — verified against ≥2 independent broker sources (`get_equity_positions`, `get_equity_orders`, `get_portfolio` cash cross-check) before writing, round-tripped through save/load after; both needed a direct script, not the CLI (`record-fill` cannot correct an already-open position). **Still open**: an automated reconcile-and-correct step, not a third one-off manual patch, is the right systemic fix — flagged in the second fix commit (`d56fa8b`), not yet built. See `docs/RFC_INTRADAY_LIVE_STOPS.md` §3, §5 non-goal 4 |
