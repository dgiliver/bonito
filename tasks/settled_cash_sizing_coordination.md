...
- PR **#28** opened (`claude/settled-cash-sizing` → main). `pr-reviewer` round 1: **NEEDS_WORK**, 1 real MAJOR + minors.
  - **MAJOR (fail-open):** `min(cash, NaN)` returns cash → a malformed `settled_buying_power` silently reverted to UNCAPPED. **Fixed:** `generate_intents` fails closed on non-finite (zero spend, exits unaffected); `live run` rejects non-finite/negative at the input boundary. Mutation-verified (neuter `isfinite` → NaN test reddens).
  - **MAJOR (doc field unverified):** reviewer couldn't verify `get_portfolio.buying_power` from sandbox — but I verified it LIVE against ••••8597 earlier this session (returns `buying_power:{buying_power:"23.77"}`, the settled figure excluding unsettled T+1). Doc clarified (nested field + settled semantics + "not ledger/total cash"); stale `get_accounts` D4 TODO removed.
  - **MINORs:** D4 log now conditional on no-cap; CLI help reworded; paper-mode warn added; +test for `position_pct_equity` composition. **NIT** (coordination doc): kept as audit trail.
  - Fix commit **47806e8**. Full fast suite 898 pass; `tests/test_live_runner.py` 110 → 113; ruff clean; diff surgical (+199/−14). Re-review dispatched.
  - Process note: an in-place `git checkout` mutation-restore clobbered the *uncommitted* NaN fix mid-verify; caught it (NaN test red after "restore"), re-applied, and switched to copy-based restore. No bad state shipped.
