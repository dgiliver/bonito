...
## Account replay vs per-symbol backtests (2026-06-28)

- **The account replay (`bonito live backtest-account`) is the only honest judge for any live-config decision; per-symbol engine backtests systematically mislead and have now reversed three times.** (1) The SPY-200 regime gate looks like dead weight in calm tapes, but removing it trips the 25% kill switch in 2022 and the *account* dies (+485% → −19.5%, 0% holdout). (2) The per-symbol overrides include GOOGL, whose own attribution is −$52, yet stripping all three degrades the account (Sharpe train 1.04→0.93, holdout 2.50→2.33) because IREN (+$1,544) and AAPL (+$234, protective) dominate and a per-symbol view can't price the interaction. (3) The same pattern caught the EMA exit rule earlier (−$1,684 attributed, but removing it made the account worse).
- **The three mechanisms a per-symbol view structurally cannot see:** the portfolio **kill switch** (one symbol's drawdown halts the whole account, zeroing every other symbol's forward P&L), **capital competition** (shared slots/cash; one symbol's entries crowd out another's), and **concentration** (account DD and Sharpe are properties of the combined curve). Raw per-symbol P&L attribution is a diagnostic, never a decision rule.
- **Corollary for "bench the weak symbol" intuitions:** a symbol with marginally negative attribution (GOOGL, −$52 ≈ 1% of starting capital) is almost always inside account-level noise. Pre-register a noise floor (≥0.05 account Sharpe on BOTH train and holdout) and expect "inconclusive → keep status quo" as the honest, common verdict. Confirmed directly: Q4 isolated GOOGL specifically (status-quo vs default vs entry_blocklist) and neither alternative cleared the floor on both windows — default helped holdout but hurt train, entry_blocklist helped train but was flat on holdout. The −$52 stayed noise under direct test, not just by analogy to the larger override bundle.
