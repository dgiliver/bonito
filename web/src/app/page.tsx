...
const AdvancedChart = dynamic(() => import("@/components/chart/AdvancedChart"), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-full" style={{ background: "var(--bg-primary)" }}>
      <div className="animate-spin rounded-full h-8 w-8 border-2 border-t-transparent" style={{ borderColor: "var(--accent-primary)" }} />
    </div>
  ),
});

// Unified Analysis View - combines chart + AI assistant
const AnalysisView = dynamic(() => import("@/components/analysis/AnalysisView"), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-full" style={{ background: "var(--bg-primary)" }}>
      <div className="animate-spin rounded-full h-8 w-8 border-2 border-t-transparent" style={{ borderColor: "var(--accent-primary)" }} />
