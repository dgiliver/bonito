...
export default function Strategies() {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg-primary)" }}>
      {/* Header Bar */}
      <header className="px-4 py-2 border-b flex items-center gap-3" style={{ borderColor: "var(--border-color)" }}>
        {/* Strategy Dropdown */}
        <div className="relative">
          ...
          >
            <span className="font-medium">{selectedStrategy?.name || "Select strategy"}</span>
              ...
              {loading ? (
                ...
                strategies.map((strategy) => (
                  <button
                    ...
                    style={{
                      background: selectedStrategy?.id === strategy.id ? "var(--bg-tertiary)" : "transparent",
                    }}
                  >
                    ...
                    {strategy.last_backtest && (
                      <span
                        className="text-xs"
                        style={{
                          color: strategy.last_backtest.total_return >= 0 ? "var(--accent-success)" : "var(--accent-danger)",
                        }}
                      >
        ...
        {/* Run Button */}
        <button
          onClick={() => selectedStrategy && handleBacktest(selectedStrategy.name)}
          disabled={!selectedStrategy || backtesting}
          className="btn btn-primary text-sm px-4 py-1.5"
          style={{ opacity: !selectedStrategy || backtesting ? 0.5 : 1 }}
        >
          {backtesting ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
          <span className="ml-1.5">{backtesting ? "Running..." : "Run Backtest"}</span>
        </button>

        {/* Refresh */}
        <button onClick={loadStrategies} className="p-1.5 rounded hover:bg-[var(--bg-secondary)] ml-auto" disabled={loading}>
          ...
          {backtestResult ? (
            <>
              ...
              {backtestResult.equity_curve && backtestResult.equity_curve.length > 0 && (
                <div className="flex-1 min-h-0" style={{ background: "var(--bg-secondary)" }}>
                  <EquityChart
        ...
        {backtestResult && (
          ...
          >
            ...
            <div className="p-4 border-b" style={{ borderColor: "var(--border-color)" }}>
              <h3 className="text-sm font-medium mb-3" style={{ color: "var(--text-secondary)" }}>Performance</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs" style={{ color: "var(--text-muted)" }}>Return</p>
                  <p
                    className="text-lg font-bold"
                    style={{ color: backtestResult.performance.total_return >= 0 ? "var(--accent-success)" : "var(--accent-danger)" }}
                  >
                    {formatPercent(backtestResult.performance.total_return)}
                  </p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: "var(--text-muted)" }}>Sharpe</p>
              ...
              {/* Trade Stats */}
              <div className="mt-3 pt-3 border-t flex justify-between text-sm" style={{ borderColor: "var(--border-color)" }}>
                <span style={{ color: "var(--text-muted)" }}>Trades</span>
                <span>
                  <span style={{ color: "var(--text-primary)" }}>{backtestResult.trades.total}</span>
                  <span style={{ color: "var(--text-muted)" }}> (</span>
                  <span style={{ color: "var(--accent-success)" }}>{backtestResult.trades.winning}W</span>
                  <span style={{ color: "var(--text-muted)" }}>/</span>
                  <span style={{ color: "var(--accent-danger)" }}>{backtestResult.trades.losing}L</span>
                  <span style={{ color: "var(--text-muted)" }}>)</span>
                </span>
              </div>
            </div>
            ...
            {/* Trade Log */}
            {backtestResult.trade_log && backtestResult.trade_log.length > 0 && (
              <div className="flex-1 p-4 min-h-0 flex flex-col">
                <TradeLog trades={backtestResult.trade_log} className="flex-1 min-h-0" />
              </div>
            )}
          </div>
        )}
